...
func main() {
	...
	signature, _ := privateKey.SignMessage(message, extraData, true, false)
	signatureBytes, _ := signature.Serialize()
	fmt.Printf("Signature: %x\n", signatureBytes)
	err := publicKey.VerifySignature(message, extraData, signature, true, false)
	...
	signature2, _ := privateKey2.SignMessage(message, extraData, true, false)
	signatureBytes2, _ := signature2.Serialize()
	fmt.Printf("Signature 2: %x\n", signatureBytes2)
	err = publicKey2.VerifySignature(message, extraData, signature2, true, false)
	...
	err = aggergatedPublicKey.VerifySignature(message, extraData, aggergatedSignature, true, false)
	fmt.Printf("Aggregated verified: %t\n", err == nil)
	err = publicKey.VerifySignature(message, extraData, aggergatedSignature, true, false)
