...
type MessageResolver interface {
	ResolveMessage(message *pkg.Message) bool
}

// BaseRouter forwards messages from their source to their destination
type BaseRouter struct {
	registry map[pkg.ChainId]MessageResolver
	lock     *sync.RWMutex
}

func NewRouter() *BaseRouter {
	return &BaseRouter{
		registry: make(map[pkg.ChainId]MessageResolver),
		lock:     &sync.RWMutex{},
	}
}

// Send passes a message to the destination Writer if it exists
func (r *BaseRouter) Send(msg *pkg.Message) error {
	r.lock.Lock()
	defer r.lock.Unlock()
...
func (r *BaseRouter) Register(id pkg.ChainId, w MessageResolver) {
	r.lock.Lock()
	defer r.lock.Unlock()
